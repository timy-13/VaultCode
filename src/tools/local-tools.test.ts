...
test("spawn_agent returns structured child-agent results through an injected runner", async () => {
  ...
  try {
    const fakeRunner: SubAgentRunner = {
      ...
      async requestShutdown(sessionId: string) {
        return {
          id: sessionId,
          parentSessionId: "parent-session-1",
          createdAt: "2024-01-01T00:00:00.000Z",
    ...
    const teamStatusResult = await adapter.executeTool(
      {
        id: "call-5bb",
        name: "team_status",
        input: {},
    ...
    const shutdownResult = await adapter.executeTool(
      {
        id: "call-5e",
    ...
    const cleanupResult = await adapter.executeTool(
      {
        id: "call-5f",
